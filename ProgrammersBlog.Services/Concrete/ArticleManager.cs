...
namespace ProgrammersBlog.Services.Concrete
{
    public class ArticleManager : IArticleService
    {
        ...
        {
            var article = await _unitOfWork.Articles.GetAsync(c => c.Id == articleId, a => a.User, a => a.Category);
            if (article != null)
            {
        ...
        public async Task<IResult> Add(ArticleAddDto articleAddDto, string createdByName)
        {
            ...
            article.UserId = 1;
            await _unitOfWork.Articles.AddSync(article);
            await _unitOfWork.SaveAsync();
            return new Result(ResultStatus.Succes, $"{articleAddDto.Title} başlıklı makale başarıyla eklendi.");
        }
